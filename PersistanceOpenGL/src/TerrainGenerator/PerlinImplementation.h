...
namespace siv
{
	...
	namespace perlin_detail
	{
        ...
        inline void Shuffle(RandomIt first, RandomIt last, URBG&& urbg)
        {
           ...
           URBG local_urbg = std::forward<URBG>(urbg); // Create a local copy to avoid moving from the original object

           for (RandomIt it = first + 1; it < last; ++it)
           {
               const std::uint64_t n = static_cast<std::uint64_t>(it - first);
               std::iter_swap(it, first + static_cast<difference_type>(Random(n, local_urbg)));
           }
        }
		//
		////////////////////////////////////////////////
